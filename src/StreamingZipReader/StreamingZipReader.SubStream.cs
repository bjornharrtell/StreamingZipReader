...
{
    private sealed class SubStream(Stream stream, long length) : Stream
    {
        ...
        private static Exception CreateDetachedStreamException() =>
            new InvalidOperationException($"A stream returned from {nameof(StreamingZipReader)}.{nameof(GetCurrentEntryStream)} may not be used after calling {nameof(StreamingZipReader)}.{nameof(MoveToNextEntryAsync)} again.");
        ...
        public override int Read(Span<byte> buffer)
        {
            ...
            // Check if this stream instance is at the end even though the inner stream may not be.
